...
public class FeignUtil {
    ...
    public Map<String, String> getSystemConfigMap(String token) {
        ...
        // 从 Redis 中获取的 SystemConf，或者通过 feign 获取
        ...
        if (StringUtils.isNotEmpty(jsonResult)) {
            ...
            if (resultTempMap.get(SysConf.CODE) != null && SysConf.SUCCESS.equals(resultTempMap.get(SysConf.CODE).toString())) {
                ...
                // 将从 token 存储到 redis 中，设置 30 分钟后过期
    ...
    public SystemConfig getSystemConfig() {
        ...
        // 判断是否是 web 端发送过来的请求【后端发送过来的 token 长度为 32】
        boolean isWebRequest = SysConf.WEB.equals(platform) || (paramsToken != null && paramsToken.length() == Constants.THIRTY_TWO);
        if (isWebRequest) {
            // 如果是调用 web 端获取配置的接口
        ...
        if (systemConfigMap == null) {
            ...
            // 判断 Minio 服务是否存在异常
            if (EOpenStatus.OPEN.equals(uploadMinio) && (StringUtils.isEmpty(minioEndPoint)
                    || StringUtils.isEmpty(minioPictureBaseUrl) || StringUtils.isEmpty(minioAccessKey)
    ...
    /**
     * 通过 Web 端的 token 获取系统配置文件 【传入 Admin 端的 token】
     *
     * @param token
     * @return
     */
    public Map<String, String> getSystemConfigByWebToken(String token) {
        ...
        // 从 Redis 中获取的 SystemConf 或者 通过 feign 获取的
        Map<String, String> resultMap = new HashMap<>();
        // 从 Redis 中获取内容
        String jsonResult = stringRedisTemplate.opsForValue().get(RedisConf.SYSTEM_CONFIG);
        // 判断 Redis 中是否有数据
